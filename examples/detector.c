	...
	char* outfile)
{
	...
	int coco = 0;
	int imagenet = 0;
	if (strcmp(type, "coco") == 0) {
		if (!outfile) outfile = "coco_results";
		snprintf(buff, 1024, "%s/%s.json", prefix, outfile);
		fp = fopen(buff, "w");
		fprintf(fp, "[\n");
		coco = 1;
	} else if (strcmp(type, "imagenet") == 0) {
	...
	for (i = nthreads; i < m + nthreads; i += nthreads) {
		...
		for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
			...
			copy_cpu(NetworkVolume(net), val_resized[t].data, 1, input.data, 1);
			flip_image(val_resized[t]);
			copy_cpu(NetworkVolume(net), val_resized[t].data, 1,
				input.data + NetworkVolume(net), 1);
...
int NetworkVolume(const network* network) {
	return network->w * network->h * network->c;
}
