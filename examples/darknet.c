...
extern void test_detector(char *datacfg, char *cfgfile, char *weightfile, 
    char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen);
...
int main(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[1], "detect")) {
        char coco_data_cfg [256];
        FileInDarknet(coco_data_cfg, 256, "cfg/coco.data");
        float thresh = find_float_arg(argc, argv, "-thresh", .5);
        char *filename = (argc > 4) ? argv[4]: 0;
        char *outImage = find_char_arg(argc, argv, "-out", 0);
        int full_json = find_arg(argc, argv, "--full-json");
        test_detector(coco_data_cfg, argv[2], argv[3], filename, thresh,
            .5, outImage, full_json);
...
char* FileInHome(char* buffer, size_t buffer_size, const char* relative_path)
{
    ...
    return strcat(strcpy(buffer, "/"), relative_path);
}

char* FileInCV(char* buffer, size_t buffer_size, const char* relative_path)
{
    strncpy(buffer, getenv("CV_HOME"), buffer_size);
